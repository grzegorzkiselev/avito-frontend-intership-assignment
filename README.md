...
1. При пагинации в 10 страниц количество страниц 102
  25 — 41
  50 — 21
  100 — 11

2. При пагинации в 10 и диапазоне цен до 1000 — 19 страниц
    + просмотры > 4 — 10 страниц
    + лайки > 4 — 3 страницы
    При переключении страниц убедиться, что элементы действительно переключаются
    Пагинация 25 — 1 страница

    + сбросить фильтрацию лайков
    + при поиске слова sed отображается 2 страницы результатов, на второй 6
        переключение страниц *действительно* переключает показываемые данные
        смена пагинации *действительно* приводит к показу 16 элементов на странице
...
1. При пагинации в 10 страниц количество страниц 101
  25 — 41
  50 — 21
  100 — 11

2. При активированных разных режимах пагинации сортировка продолжает работать
  + при выборе сортировки по убыванию суммы и статусу transport первый элемент списка имеет сумму 49982
  + при пагинации 100 — 3 страницы резульатов
  + переключение страниц *действительно* работает
  + при добавлении параметра forItem=8 и статуса Recieved — первый заказ имеет товар «Новый айфон»
  + смена пагинации на 10 *действительно* меняет размер выдачи. Получается 3 страницы
  + страницы *действительно* переключаются

  + после переключения на 3 страницу в адресной строке:
    http://localhost:5173/orders?paginationSize=50&page=3&sortLabel=%D0%A1%D0%BD%D0%B0%D1%87%D0%B0%D0%BB%D0%B0+%D1%81+%D0%B1%D0%BE%D0%BB%D1%8C%D1%88%D0%B5%D0%B9+%D1%81%D1%83%D0%BC%D0%BC%D0%BE%D0%B9&statusLabel=Received

##### Общее

+ если переключиться на последнюю страницу и увеличить размер пагинации — выбранная страница подстроится под количетсво страниц
...
- [~] Рефакторинг
  - [ ] Есть явное дублирование кода между заказами и объявлениями — из-за ограниченного времени я не стал начинать с продумывания абстракций, решив, что сперва будет правильно добиться верного поведения, потом его унифицировать
    - [x] Начал выводить общую логику в классы
    - [x] Вывел общий редьюсер
  - [ ] Не слишком чистое разделение компонентов и логики
...
- [x] Некоторые значения приходится обрабатывать на клиенте, например поиск и отображение заказов с определённым элементом. Здесь не приходится рассчитывать на серверную пагинацию, приходится нарезать на клиенте.
  ...
  - [ ] Добавить лоадеры на время фильтрации на странице объявлений
